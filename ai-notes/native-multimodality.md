...
![Typical pipeline](images/Typical%20pipeline.png)


**For example:**

- Images are processed by a vision encoder (CNN or ViT).  
- Audio is converted to text via speech recognition.  
- The resulting representations are mapped into a language model through adapter layers.  
...
**Key limitation:**  
Fine-grained spatial details, tone, emotion, background sound, and visual composition are often discarded during intermediate conversions.

### Gemini’s Native Multimodality

Gemini takes a fundamentally different approach.  

Instead of translating everything into text, Gemini treats visual, auditory, and textual signals as equally valid symbolic inputs. Pixels, sound waves, and words are all converted into tokens and processed jointly.
...
- **Unit:** image patches  
- **Typical patch size:** 16×16 or 32×32 pixels  
...
- Sample raw waveform (e.g., every 20 ms)  
- Convert to frequency domain via Fourier transform  
- Encode using an audio encoder (e.g., spectrogram-based transformers)  
...
All tokens, regardless of modality, are mapped into vectors of the same dimensionality:
...
![Single AI Brain](images/Single%20AI%20Brain.png)
...
- Low-resolution global thumbnail → Few tokens, captures overall semantics (“this is a landscape”)  
- High-resolution local crops → Allocated only where detail matters
...
Unlike early ViT models that force images into square inputs (e.g., 224×224), Gemini:
...
![image dealing](images/image%20dealing.png)
...
- Extreme training cost: Requires massive compute and data.  
- Data complexity: Large-scale aligned text–image–audio–video datasets.  
- Optimization difficulty: Preventing catastrophic interference between modalities.  
...
![Systolic Array Advantage](images/Systolic%20Array%20Advantage.png)
...
- Compressing parameters (FP32 → INT8)  
